      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="text-[#8b949e] text-xs uppercase tracking-wider border-b border-[#21262d]">
            ...
            <th className="text-right py-2 px-3">Stop Dist.</th>
          </tr>
        </thead>
        <tbody>
          {trades.map((trade) => {
            const current = currentPrices[trade.symbol] || trade.entry_price;
            const pnl = current - trade.entry_price;
            const pnlPct = (pnl / trade.entry_price) * 100;
            const stopDist = Math.abs(current - trade.stop_price);
            const stopDistPct = (stopDist / current) * 100;

            return (
              <tr key={trade.id} className="border-b border-[#21262d]/50 hover:bg-[#21262d]/30">
                <td className="py-2.5 px-3 font-medium text-white">{trade.symbol}</td>
                ...
                <td className="py-2.5 px-3 text-right text-[#8b949e]">
                  ${stopDist.toFixed(2)} ({stopDistPct.toFixed(1)}%)
                </td>
              </tr>
            );
          })}
